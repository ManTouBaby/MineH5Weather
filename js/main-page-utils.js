var mainPageUtil = {
	options: {
		ACTION_COLOR: "#d74b28",
		NORMAL_COLOR: "#666666",
		subpages: ["subpage/main-subpage-news.html", "subpage/main-subpage-photo.html", "subpage/main-subpage-setting.html"]
	},
	/*简单封装了绘制原生view控件的方法、绘制内容支持font（文本，字体图标）,图片img , 矩形区域rect*/
	drawNativeView: function(id, styles, tags) {
	...
	initSubpage: function(aniShow) {
		var subpage_style = {
			...
			subpages = mainPageUtil.options.subpages,
	...
	toggleNview: function(currIndex) {
		...
		mainPageUtil.updateSubNView(currIndex, mainPageUtil.options.ACTIVE_COLOR);
		mainPageUtil.updateSubNView(currIndex + 1, mainPageUtil.options.ACTIVE_COLOR);
		// 重绘兄弟tag 反之排除当前点击的icon和text
		for(var i = 0; i < 8; i++) {
			if(i !== currIndex && i !== currIndex + 1) {
				mainPageUtil.updateSubNView(i, mainPageUtil.options.NORMAL_COLOR);
			}
		}
	},
	/*
	...
	updateSubNView: function(currIndex, color) {
		...
		nviewEvent.drawText(currTag.text, currTag.position, mainPageUtil.changeColor(currTag.textStyles, color), currTag.id);
	}
